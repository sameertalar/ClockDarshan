$(document).ready(function () {
  ...
  function showCurrentSelection() {
    ...
    $("#lableSelectedGod").html($("#selectTrackerTime option:selected").data("god"));
  }
    ...
    $.ajax({
      ...
      success: function (data, textStatus, xhr) {
        for (let i = 0; i < data.chunks.length; i++) {
          ...
          $("#lblChunkQuarter" + (data.chunks[i].id)).html(data.chunks[i].h2 + "-"+ data.chunks[i].q);
          $("#btnChunkPush" + (data.chunks[i].id)).attr("data-row",data.chunks[i].row);
