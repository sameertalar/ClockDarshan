$(document).ready(function () {
  ...
  function processBeHereNow() {
    try {
      ...
      if (__alertPlayIndicator < (min - (min % 5)) * 60 + sec + 2) {
  ...
  function playNotification(isChunk, hours, minuteChunk, sec, ms) {
    let alertMode;
    __timeInterval = 0;
    __alertPlayIndicator = minuteChunk * 60 + sec + ms;
    ...
    if (
      ...
      $.each(__dataConfig.chunkConfigs, function (index, value) {
        if (minuteChunk === value.chunk) {
          ...
          $("#displayNaam").css("color", value.color);
          __CurrentChunk = minuteChunk;
        ...
        if (minuteChunk === value.chunk) {
          if ($("#checkboxAlertWindow").is(":checked")) {
            ...
            windowPop.document.write(
              "<html><head><title>Be Here Now</title><style>body{background-color:" +
                value.color +
                ";font-family:Verdana; text-align:center;  color: aliceblue;}</style></head><body>" +
                "<h2>" +
                pad(minuteChunk, 2) +
                " </h2><h3> " +
                value.displayText +
  ...
  function getQueryString() {
    ...
    queryString +=
      "&popwindow=" + ($("#checkboxAlertWindow").is(":checked") ? "1" : "0");
  ...
  function selectProfileOnChange(isload) {
    ...
    $("#checkboxAlertWindow").prop("checked", popwindow == 1 ? true : false);
