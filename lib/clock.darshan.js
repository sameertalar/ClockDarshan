$(document).ready(function () {
    ...
    try {
      ...
      if (__alertPlayIndicator !== (min - (min % 5)) * 60 + sec) {
        ...
        if ($("#checkboxQuarterMode").is(":checked")) {
          if (min % 15 === 0 && sec === 1) {
            playNotification(true, hours, min, sec, ms);
          ...
          if (midNow) {
            playNotification(false, hours, min - (min % 5), sec, ms);
            console.log(" 🔔***** Mid  (2.5 min) alert at ", new Date());
          }
        }
  ...
  function page_Load() {
    ...
    $("#checkboxListenChant").change(checkboxAlertsOnChanged);
  ...
  function playNotification(isChunk, hours, minuteChunk, sec, ms) {
    if (!$("#checkboxAlertsOn").is(":checked")) {
      return;
    }
    ...
    // 4.Post Alerts
    popupAlertChunkName(isChunk, minuteChunk, hours % 12);
  ...
  function tryChunkClick() {
    ...
    let min = now.getMinutes();
    let hours = now.getHours();
    playNotification(true, hours, min - (min % 5), 0, 0);
    playVideo();
  }
  ...
  function tryMidClick() {
    ...
    let min = now.getMinutes();
    let hours = now.getHours();
    playNotification(false, hours, min - (min % 5), 0, 0);
  }
  ...
  function checkboxAlertsOnChanged() {
    if ($("#checkboxListenChant").is(":checked")) {
      ...
      // $("#listen-chant-alert").addClass("bg-success");
      _chantListingOn = true;
    } else {
      stopRecording();
      $("#listen-chant-lable").removeClass("bg-success");
      // $("#listen-chant-alert").addClass("alert-success");
  ...
  function startRecording() {
    recognition.onstart = function () {
      $("#listen-chant-alert").removeClass("alert-danger");
      $("#listen-chant-alert").addClass("alert-success");

      setTimeout(() => {
        $("#listen-chant-alert").html("On 🔊");
      }, 1000);
    };
      ...
      try {
        ...
        for (var i = 0; i < event.results.length; i++) {     
          ...
          //  console.log(event.results[i][0].transcript); //todo remove
            ...
            if (
              event.results[i][0].transcript.indexOf("नमः") >= 0 ||
        ...
        $("#errorMessage").html(
          $("#errorMessage").html + "Listen Chant Error" + error
        );
      }
    };
    recognition.start();
    ...
    recognition.onend = function () {
      if (_chantListingOn) {
        recognition.start();
        console.log("Speech recognition service EWSUMED!");
      } else {
        console.log("Speech recognition service disconnected");

        $("#listen-chant-alert").addClass("alert-danger");
        $("#listen-chant-alert").removeClass("alert-success");
        $("#lableChantListen").addClass("bg-warning");

        setTimeout(() => {
          $("#listen-chant-alert").html("Stopped.");
        }, 1000);
      }
    };
  ...
  function popupAlertChunkName(isChunk, minuteChunk, hours) {
  ...
  function displayTrackers() {
    $("#errorMessage").html(
      $("#selectMind option:selected").val() +
        " " +
        $("#selectEnergy option:selected").val() +
        " " +
        $("#selectWork option:selected").val()
    );
  }
  ...
  function setTrackerTime(isLoad) {
    ...
    if (isLoad) {
      ...
      $("#selectMind").val("A");
      $("#selectEnergy").val(selEnergy);
      $("#selectWork").val(selWork);
    }
  ...
  function postToGoogle() {
    ...
    $.ajax({
      ...
      success: function (data, textStatus, xhr) {
        console.log(data);
        let selection =
          $("#selectMind option:selected").text() +
          " : " +
          $("#selectEnergy option:selected").text() +
          " : " +
          $("#selectWork option:selected").text();
