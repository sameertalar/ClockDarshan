$(document).ready(function () {
  ...
  function playNotification(isChunk, hours, minuteChunk) {
    ...
    // 3.Play Notification
    if (alertMode === "Audio" || alertMode === "Text") {
  ...
  function playNotificationAudio(minuteChunk, fileName, volume) {
    let audioAlert = $("#audioAlert");

    audioAlert.attr("src", MEDIA_ROOT + "audios/" + fileName);

    audioAlert[0].play();
    //audioAlert[0].volume=volume;  //ToDo later

    console.log("Audio Played for chunk: ", minuteChunk, " file:", fileName);
  }
