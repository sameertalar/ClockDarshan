$(document).ready(function () {
  ...
  function processBeHereNow() {
    try {
      ...
      let selectedQuarterRow = $("#selectTrackerTime option:selected").data(
        "row"
      );
      ...
      if ($(".checkEdit:checked").val() != "T") {
        setTrackerTime();
        if (
          Number($("#LoggedQuarterRow").val()) + 1 <
          Number(selectedQuarterRow)
        ) {
          $("body").addClass("bag");
        } else {
        ...
        if ($("#LoggedQuarterRow").val() == $("#CurrentQuarterRow").val()) {
    ...
    $.ajax({
      ...
      success: function (data, textStatus, xhr) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          $("#chunkLablelH"+ data[i].id).html(data[i].h);
          $("#chunkLablelE"+ data[i].id).html(data[i].e);
          $("#chunkLablelW"+ data[i].id).html(data[i].w);
          $("#chunkLablelM"+ data[i].id).html(data[i].m);
          $("#hiddenTime"+ data[i].id).val(data[i].h2+"-"+data[i].q);
