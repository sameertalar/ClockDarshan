$(document).ready(function () {
  ...
  function playNotification(isChunk, hours, minuteChunk) {
    ...
    for (let i = 0; i < __dataConfig.chunkConfigs.length; i++) {
      if (minuteChunk === __dataConfig.chunkConfigs[i].chunk) {
        // $("#displayNaam").html(__dataConfig.chunkConfigs[i].displayText);
        ...
        switch (alertMode) {
          ...
          case "Text":
            ...
            //Todo
            textmessage = __dataConfig.chunkConfigs[i][controlMode + "Text"];

            let isEng =
              __dataConfig.chunkConfigs[i][controlMode + "Lang"] > "eng"
                ? false
                : true;
        ...
        // Misc
        popupAlertChunkName(isChunk, __dataConfig.chunkConfigs[i].NaamText);
  ...
  function testAudioAlert(id, type) {
    let audioAlert = $("#audioAlert");
    let controlId = id.id.replace("test", "");

    let fileName = $("#" + controlId)
      .find(":selected")
      .data("filename");

    audioAlert.attr("src", MEDIA_ROOT + type + "/" + fileName);
  ...
  function playBeforeAlertAudio(isChunk, timeInterval) {
    ...
    if (isChunk) {
      if ($("#checkboxChunkBeforeAlert").is(":checked")) {
        play = true;
        fileName = $("#selectChunkBeforeAlertAudio").val();
      }
    } else {
      if ($("#checkboxMidBeforeAlert").is(":checked")) {
        play = true;
        fileName = $("#selectMidBeforeAlertAudio").val();
      }
    }
  ...
  function selectProfileOnChange() {
    ...
    $("#selectTimeAlertMode").val(timealertmode);
    $("#selectChunkNotifyMode").val(chunknotifymode);
    ...
    $("#selectMidNotifyMode").val(midnotifymode);
