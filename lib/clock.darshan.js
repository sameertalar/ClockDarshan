$(document).ready(function () {
  ...
  function page_Load() {
    ...
    $("#audioTicker").attr("src", MEDIA_ROOT + "alerts/ticking_Water_Two_Drops.mp3");
  ...
  function playTimeAlert(hours, minuteChunk) {
    ...
    if (mode !== "Silent") {
      ...
      if ($("#checkboxBrokenMinutesTime").is(":checked")) {
      ...
      console.log("Time Played at time:", hours + ":" + minuteChunk, " mode:",mode);
    }
  }
  ...
  function tickerTick() {
    if ($("#checkboxTicker").is(":checked")) {
      if (__tickerState == parseInt($("#selectTickerDuration").val())) {
        console.log("Ticker Audio played at Ticker State:", __tickerState," at", new Date());
        // play audio;
        $("#audioTicker")[0].play();
        __tickerState = 0;
      }
  ...
  function tickerAudioChanged(controlId) {
    ...
    let fileName = MEDIA_ROOT + "alerts/" + $("#" + controlId).find(':selected').data('filename');
        ...
        if (minuteChunk === value.chunk) {
          if ($("#checkboxAlertWindow").is(":checked")) {
            ...
            windowPop.document.write(
              "<html><head><title>Be Here Now</title><style>body{background-color:"+value.color+";font-family:Verdana; text-align:center;  color: aliceblue;}</style></head><body>" +
              "<h2>" +
              pad(minuteChunk, 2) +
              " </h2><h3> " +
              value.displayText +
