$(document).ready(function () {
  ...
  $("#controlProfile").change(controlProfileOnChange);
  ...
  function processBeHereNow() {
    try {
      ...
      $("#errorMessage").innerHTML += "Unhandled Error: " + error;
    }
  }
  ...
  function populateProfileLists() {
    for (let i = 0; i < __dataProfile.length; i++) {
      if (__dataProfile[i].Selected === 1) {
        $("#controlProfile").append(
          $("<option selected></option>")
            .attr("value", __dataProfile[i].ProfileValue)
            .text(__dataProfile[i].DisplayText)
        );
      } else {
        $("#controlProfile").append(
          $("<option></option>")
  ...
  function playNotification(isChunk, hours, minuteChunk) {
    ...
    if (__dataConfig === null || __dataConfig.length < 1) {
      $("#errorMessage").innerHTML += "Unable to load json data";
    }
    if (isChunk) {
    ...
    for (let i = 0; i < __dataConfig.length; i++) {
      if (minuteChunk === __dataConfig[i].chunk) {
        $("#displayNaam").innerHTML = __dataConfig[i].NaamText;
    ...
    if (__CurrentChunk !== minuteChunk) {
      for (let i = 0; i < __dataConfig.length; i++) {
        if (minuteChunk === __dataConfig[i].chunk) {
          ...
          document.title = __dataConfig[i].NaamText;
        }
      }
    }
  }
  ...
  function pauseAllAudios() {
    try {
      ...
      $("#errorMessage").innerHTML += "pauseAllAudios Error" + error;
  ...
  function playVideo() {
    try {
      ...
      $("#errorMessage").innerHTML = "Video playing failed! " + err;
