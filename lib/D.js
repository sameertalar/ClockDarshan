$(document).ready(function () {
  ...
  const __Configuration = {
    mind: [
      { title: "Peace", column: "N", icon: "🕊️" ,fa:"fa fa-paper-plane text-success"},
      { title: "ध्यान", column: "O", icon: "🧘" ,fa:"fa fa-user-circle  text-primary"},
      { title: "अप्रमाद", column: "P", icon: "🧿" ,fa:"fa fa-eye  text-warning"},
      { title: "Pramad", column: "Q", icon: "🎳" ,fa:"fa fa-eye-slash tet-dark"},
      { title: "Buzzing", column: "R", icon: "🔥" ,fa:"fa fa-fire   text-danger"},
      { icon: "❌" ,fa:"fa fa-times  text-secondary"},
      { icon: "💤" ,fa:"fa fa-bed  text-secondary"}
    ],
    energy: [
      { title: "High", column: "I", icon: "🔋" ,fa:"fa fa-battery-full text-success"},
      { title: "Medium", column: "J", icon: "🥥" ,fa:"fa fa-battery-half  text-warning"},
      { title: "Low", column: "K", icon: "⚪" ,fa:"fa fa-battery-empty  text-danger"},
      { icon: "❌" ,fa:"fa fa-times  text-secondary"},
      { icon: "💤" ,fa:"fa fa-bed  text-secondary"}
    ],
    work: [
      { title: "Spiritual", column: "C", icon: "🪔" ,fa:"fa fa-certificate text-info"},
      { title: "Value Add", column: "D", icon: "💎" ,fa:"fa fa-diamond  text-primary"},
      { title: "Health", column: "E", icon: "🌿" ,fa:"fa fa-leaf  text-success"},
      { title: "Duty ", column: "F", icon: "👪" ,fa:"fa fa-life-ring  text-warning"},
  ...
  function getFaClass(icon, type) {
   
    return  __Configuration[type].find((x) => x.icon === icon).fa;
    ...
    $.ajax({
      ...
      success: function (data, textStatus, xhr) {
        for (let i = 0; i < data.chunks.length; i++) {
          ...
          $("#lblChunkM" + data.chunks[i].id).addClass(getFaClass(data.chunks[i].m,"mind"));
          $("#lblChunkW" + data.chunks[i].id).addClass(getFaClass(data.chunks[i].w,"work"));
          $("#lblChunkE" + data.chunks[i].id).addClass(getFaClass(data.chunks[i].e,"energy"));
  ...
  function Mind_Change() {
    $("#centerCoreM").removeClass()
    $("#centerCoreM").addClass(
  ...
  function Work_Change() {
    $("#centerCoreW").removeClass()
    $("#centerCoreW").addClass(
  ...
  function Energy_Change() {
    $("#centerCoreE").removeClass()
    $("#centerCoreE").addClass(
