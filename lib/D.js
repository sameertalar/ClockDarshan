...
const _gods = [
  ...
  const __Configuration = {
    mind: [
      ...
      { icon: "❌", fa: "fa fa-exclamation-triangle  text-danger" },
  ...
  function postToGoogle(update) {
    ...
    $.ajax({
      ...
      error: function (xhr, error_text, statusText) {
        $("#errorMessage").html(error_text.lq);
  ...
  function showCurrentSelection() {
    ...
    $("#lableNextGod").html(
        _gods[($.inArray($("#selectTrackerTime option:selected").data("god"), _gods) + 1) % _gods.length]      
    );
