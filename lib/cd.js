$(document).ready(function () {
  ...
  function populateConfigSelectLists() {
    ...
    $.each(__dataConfig.trackerConfigs.mind, function (index, value) {
      ...
      */
      ...
      $("#containerMind")
        ...
        .append(
          `<input type="radio" class="radioMind form-check-input mt-4   " id="radioMind${value.code}" name="radioMind" value="${value.code}">`
        )
        .append(`</div> `);
    });
    ...
    $.each(__dataConfig.trackerConfigs.energy, function (index, value) {
      $("#containerEnergy")
        ...
        .append(
          `<input type="radio" class="radioEnergy form-check-input  mt-4  " id="radioEnergy${value.code}" name="radioEnergy" value="${value.code}">`
        )
        .append(`</div> `);
    });
    ...
    $.each(__dataConfig.trackerConfigs.work, function (index, value) {
      $("#containerWork")
        ...
        .append(
          `<input type="radio" class="radioWork form-check-input  mt-4  " id="radioWork${value.code}" name="radioWork" value="${value.code}">`
        )
        .append(`</div> `);
    });
  ...
  function showTimeElaspeProgress(min, sec) {
    ...
    $("#bodyCD").css("background-size", elaspePercent + "%");
...
*/
  ...
  function loadTrackerSheetFrame() {
    if ($("#containerTrackerSheet").is(":visible")) {
      ...
      $("#containerTrackerSheet").show();

      var trackerHeight = $(window).height() - $("#containerControls").height();

      $("#containerTrackerSheetscroll").height(trackerHeight - 150);
      $("#frameTrackerSheet").height(trackerHeight - 50);
  ...
  function postToGoogle() {
    ...
    $.ajax({
      ...
      success: function (data, textStatus, xhr) {
        console.log(data);
        let selection = paramMind + " : " + paramEnergy + " : " + paramWork;
